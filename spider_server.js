...
for ( var i = 1; i < configs.spider_count + 1; i++) {
  var spider = new Spider('spider_' + i);
  spider.on('spider_ok', function(task) {
    ...
    if (task.new_task_id > 0) {
      ...
      //console.log('NEWTASK: ' + this.name + ' : ' + new_task + ' will add to queue');
  ...
  spider.on('spider_error', function(data) {
    //console.log('TASK_ERROR: ' + this.name + ' : task-error emmited : ' + data.task.original_task.uri + ' ERROR_MSG: ' + data.msg);
