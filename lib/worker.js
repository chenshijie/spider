...
Worker.prototype.fetchPageContent = function(task, callback) {
  ...
  if (task.pageContentCached) {
    ...
    function(error, response, body) {
      if (error) {
        console.log(utils.getLocaleISOString() + ' ' + JSON.stringify(error) + ' : ' + task.urlInfo.url);
        console.log(utils.getLocaleISOString() + ' FETCH_ERROR : ' + task.original_task.uri);
        var error = {
 ...
 * @returns
 */
Worker.prototype.fetchPageContentViaHttp = function(task, callback) {
  if (task.debug) {
    console.log('----------> fetchPageContentViaHttp ' + task.urlInfo.id + '<-----------');
  }
  if (task.pageContentCached) {
    callback(null, task);
  } else {
    var temp = url.parse(task.urlInfo.url);
    var options = {
      host: temp.host,
      path: temp.path,
      port: temp.port || 80
    ...
    var req = http.get(options, function(res) {
      ...
      res.on('error', function(e) {
        console.log(utils.getLocaleISOString() + ' ' + JSON.stringify(e) + ' : ' + task.urlInfo.url);
        console.log(utils.getLocaleISOString() + ' FETCH_ERROR : ' + task.original_task.uri);
        var error = {
          error: 'FETCH_URL_ERROR',
          msg: 'response error'
    ...
    req.setTimeout(30000, function() {
      console.log(utils.getLocaleISOString() + ' FETCH_URL_TIME_OUT : ' + task.original_task.uri);
      req.abort();
      var error = {
        error: 'FETCH_URL_TIME_OUT',
  ...
  if (task['pageContentUnchanged'] == false) {
    ...
    task.mysql.store_page_content(task.id, in_time, task.urlInfo.stock_code, task.table, JSON.stringify(meta), task.pageContent, function(id) {
      if (id == 0) {
        ...
        task['save2DBOK'] = true;
        task['new_task_id'] = id;
        //Save Page Content to Redis for Parser
        var cache_key = 'PAGECONTENT:'+id;
        task.redis.set(cache_key, task.pageContent);
        //End
        callback(null, task);
      }
    });
