...
var Spider = function(spider_name) {
  ...
  var onRun = function(taskinfo) {
    if (self.status == 'running') {
      console.log(self.name + 'is running ,task:' + taskinfo.task.original_task.uri + ' will be ignored!');
      return;
    }
    self.status = 'running';
    // console.log(self.name + ' run');
    self.task = taskinfo.task;
    self.db = taskinfo.db;

    if (self.task.table == 'url') {
      self.db.get_page_content_count(self.task.url_id, self.task.table, function(count) {
        if (count == -1) {
  ...
  var onCheckSuccess = function(check_result) {
    self.db.get_url_info(self.task.table, self.task.id, function(result) {
      ...
      if (result.length == 0 || result[0].url == '' || result[0].site == '' || result[0].type == '') {
        ...
        self.emit('spider_ok', { task : self.task, msg : ' can not read url info by id ' + task.id, new_task_id : 0 });
        self.status = 'waiting';
      } else {
