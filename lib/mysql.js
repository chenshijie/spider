...
MySqlClient.prototype.get_base_url = function(fetch_time, count, cb) {
  this.client.query('SELECT * FROM baseurl WHERE type = "list" and fetch_time < ? limit ?', [ fetch_time, count ], function(err, results, fields) {
    if (err) {
      cb(err);
    } else {
      cb(results);
...
MySqlClient.prototype.get_base_url_bulletin = function(fetch_time, count, cb) {
  this.client.query('SELECT * FROM baseurl WHERE type = "bulletin" and fetch_time < ? limit ?', [ fetch_time, count ], function(err, results, fields) {
    if (err) {
      cb(err);
    } else {
      cb(results);
